...
# an outdir directory. Works on ID83 contexts.
...
sig_dn_fi <- paste(input, "ID83/Suggested_Solution/De_Novo_Solution/De_Novo_Solution_Signatures_SBSINDEL.txt", sep ="/")
act_dn_fi <- paste(input, "ID83/Suggested_Solution/De_Novo_Solution/De_Novo_Solution_Activities_SBSINDEL.txt", sep="/")
sig_decomp_fi <- paste(input, "ID83/Suggested_Solution/Decomposed_Solution/Decomposed_Solution_Signatures_SBSINDEL.txt", sep="/")
act_decomp_fi <- paste(input, "ID83/Suggested_Solution/Decomposed_Solution/Decomposed_Solution_Activities_SBSINDEL.txt", sep="/")
...
save_plot(paste(outdir, "denovo_ID83_activity_figure.pdf", sep = "/"),dn_actplot, base_asp = 1.6, base_height = 6)

## De novo counts summary TSV
dn_summary <- tidysig::per_sample_summary(dn_acts)
write_tsv(dn_summary, paste(outdir, "denovo_ID83_activity_summary.tsv", sep="/"))
...
save_plot(paste(outdir, "decomposition_ID83_activity_plot.pdf", sep="/"), decomp_actplot, base_asp=1.6, base_height=6)

## Decomposition counts summary TSV
decomp_summary <- tidysig::per_sample_summary(decomp_acts)
write_tsv(decomp_summary, paste(outdir, "decomposition_ID83_activity_summary.tsv", sep="/"))
